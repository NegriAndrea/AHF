...
{
#ifdef USE_PTYPE
  // here you can define whatever particle types you like to use for the merger tree construction:
  // PDM     = dm particles
  // PSTAR   = star particles
  // PGAS    = gas particles
  // Note, when using stars, you must have compiled the code with -DUSE_PIDMAP
  // Further, Ptype is positive whereas PX is negative (cf. define.h)
  
  // this uses only the dm particles for the tree construction
  if(Ptype == abs(PDM))
    return(TRUE);
  else
    return(FALSE);
    
  // this, for instance, uses both DM and STARS
//  if((Ptype == abs(PDM)) || (Ptype == abs(PSTAR)))
//    return(TRUE);
//  else
//    return(FALSE);
