 ...
 *
 *
 *  This code was written back in 2009 by AK and never really used for any publication,
 *  but I felt like leaving it in here just in case...
 *==================================================================================================*/
 ...
 *                                  THE STRUCTURES
 *
 *  note, this structure is different to the one used by MergerTree
 *  and hence the distinct name "typedef struct MTREER"
 *-------------------------------------------------------------------------------------*/
typedef struct MTREER *MTREERptr;
{
  uint64_t  haloid;
  uint64_t  npart;
  uint64_t  nprog;
...
double   omega0, lambda0, hubble;

/*-------------------------------------------------------------------------------------
 *                                   PROTOTYPES
 *-------------------------------------------------------------------------------------*/
int      merger_rates     (char Filename[MAXSTRING], int);
MTREER   *read_mtree       (char *, uint64_t *);
uint64_t count_merger     (MTREER *, uint64_t, uint64_t *);
uint64_t count_progs      (MTREER *, uint64_t);
void     write_merger     (FILE *, MTREER *, uint64_t);
void     write_progs      (FILE *, MTREER *, uint64_t);
void     read_zred        (char zredfile[MAXSTRING], int , int);
int      test_zredfile    (char zredfile[MAXSTRING]);
#ifdef HALOID_RESTRICTION
void     read_haloids        (char haloidsfile_izred[MAXSTRING], MTREER *, uint64_t);
void     write_haloids       (char haloidsfile_izred[MAXSTRING], MTREER *, uint64_t);
 ...
 *
 *==================================================================================================*/
MTREER *read_mtree(char *prefix, uint64_t *nhalos)
{
  ...
  *nhalos = 0;
  ...
  while(!feof(fpin)) {
    // add halo to mtree[]
    (*nhalos)++;
    mtree = (MTREERptr) realloc(mtree, (*nhalos)*sizeof(MTREER));
 ...
 *
 *==================================================================================================*/
uint64_t count_merger(MTREER *mtree, uint64_t nhalos, uint64_t *nhalos_M0)
{
 ...
 *
 *==================================================================================================*/
void write_merger(FILE *fpout_merger, MTREER *mtree, uint64_t nhalos)
{
 ...
 *
 *==================================================================================================*/
uint64_t count_progs(MTREER *mtree, uint64_t nhalos)
{
	uint64_t ihalo;
	uint64_t nprogs;
 ...
 *
 *==================================================================================================*/
void write_progs(FILE *fpout, MTREER *mtree, uint64_t nhalos)
{
	uint64_t ihalo;
  int64_t  dnpart;
  double   fMacc;
 ...
 * read_haloids:
 *==================================================================================================*/
void read_haloids(char haloidsfile_izred[MAXSTRING], MTREER *mtree, uint64_t nhalos)
{
  FILE    *fp;
  char     line[MAXSTRING];
  ...
  qsort((void *)mtree, nhalos, sizeof(MTREER), qcompareHaloIDs);
  ...
  while(!feof(fp)) {
    ...
    itmp_mtree = (MTREER *)bsearch(&(haloid), mtree, nhalos, sizeof(MTREER), bcompareMtreeIDs);
 ...
 * write_haloids:
 *==================================================================================================*/
void write_haloids(char haloidsfile_izred[MAXSTRING], MTREER *mtree, uint64_t nhalos)
{
  FILE    *fp;
  uint64_t ihalo, iprog;
...
int qcompareHaloIDs(const void *mtree1, const void *mtree2)
{
	...
	n1 = ((MTREER *)mtree1)->haloid;
	n2 = ((MTREER *)mtree2)->haloid;
...
{
  const uint64_t haloid = ((MTREER *)mtree)->haloid;
