/* $Id: io_mgizmo.c,v 1.20 2007/12/10 13:13:08 knolli Exp $ */
...
\**********************************************************************/
extern io_mgizmo_t
io_mgizmo_open_withoutNCPUREADING_EQ_NFILES(io_logging_t log,
                char *fname,
                ...
                uint32_t reader)
{
  ...
  int32_t *slen;
	...
	f = (io_mgizmo_t)malloc(sizeof(io_mgizmo_struct_t));
	if (f == NULL) {
		io_logging_memfatal(log,  "io_mgdaget_open(): sizeof(io_mgizmo_struct_t)");
		return NULL;
	}
  
	/* Start filling the structure */
  
	/* Okay, we are a Multiple Gizmo file */
	f->ftype = IO_FILE_MGIZMO;
  ...
  if (limit.rlim_cur < f->numfiles+NFILES_EXTRA)
   {
	...
	/* Glue the files into the MGizmo structure */
	f->files = (io_gizmo_t *)malloc( sizeof(io_gizmo_t)*(f->numfiles));
	if (f->files == NULL) {
    io_logging_memfatal(log,  "io_mgizmo structure (2)");
		for (i=0; i<f->numfiles; i++)
			free(fnames[i]);
		free(fnames);
		free(f->stem);
...
#		ifdef WITH_MPI
      /* TODO
       * THIS	IS JUST A NASTY HACK TO PREVENT io_gizmo.c FROM REDOING
       * THE MPI-SPLIT. CALLED FROM HERE IT IS ONLY SUPPOSED TO ACT AS
       * A DUMMY INTERFACE TO A GIZMO FILE.
       * TODO
       */
      (f->files)[i] = io_gizmo_open(log, fnames[i], swapped, mode, f->size+1);
#		else
      (f->files)[i] = io_gizmo_open(log, fnames[i], swapped, mode, reader);
#		endif
  ...
  /* we nevertheless need to generate all the file structures ready to receive the broadcast */
    ...
    for (i=0; i<f->numfiles; i++) {
      ((f->files)[i]) = (io_gizmo_t)calloc(sizeof(io_gizmo_struct_t),1);
  ...
  for (i=0; i<f->numfiles; i++) {
    MPI_Bcast((f->files)[i], sizeof(io_gizmo_struct_t), MPI_BYTE, 0, f->mycomm);
...
#		endif
  ...
  for (i=0; i<f->numfiles; i++) {
    MPI_Bcast((f->files)[i], sizeof(io_gizmo_struct_t), MPI_BYTE, i, f->mycomm);
                ...
                io_mgizmo_t f)
{
  ...
  if(f->rank_mycomm == 0) {
    ...
    for (i=0; i<f->numfiles; i++) {
      ((f->files)[i])->header = (io_gizmo_header_t)malloc((size_t)GIZMO_HEADER_SIZE+1);
    }
  }
    
  /* Broadcast the header of each file from rank==0 to all other MPI tasks */
  for (i=0; i<f->numfiles; i++) {
    MPI_Bcast(((f->files)[i])->header,((size_t)GIZMO_HEADER_SIZE+1),MPI_BYTE,0,f->mycomm);

    /* io_gizmo_init() also set some more relevant values in f */
                    ...
                    io_file_strg_struct_t strg)
{
  ...
  f->maxpos[2] = -1e40;
	...
	part_read = io_mgizmo_readpart_raw(log, f, pskip, pread, strg);
	if (part_read != pread) {
		return UINT64_C(0);
	}
                        ...
                        io_file_strg_struct_t strg)
{
	...
	for (i=0; i<f->numfiles; i++) {
		 ...
		 */
		if (io_gizmo_get(log, f->files[i], IO_FILE_GET_NOPART, &tmp) != true) {
    ...
    if(i == f->rank_mycomm) {
      ...
      partread = io_gizmo_readpart_raw(log, f->files[i], pskip_file, pread_file, strg);
...
#if (defined WITH_MPI && defined NCPUREADING_EQ_NFILES)
 {
  ...
  /* note, this reduction is in fact repeated by io_gizmo_scale_global() but what the heck... */
               ...
               void *res)
{
	...
	switch (what) {
		...
		case IO_FILE_GET_DOUBLE:
			io_logging_warn(log, INT32_C(1),
			                "Gizmo files don't store the use of "
...
io_mgizmo_resetscale(io_logging_t log,
	...
	for (i=0; i<f->numfiles; i++) {
		io_gizmo_resetscale(log, f->files[i], posscale, weightscale);
	}
	f->posscale = posscale;
